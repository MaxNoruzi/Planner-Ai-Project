        ...
        for successor in successors:
            if goal_test(successor, goal_state):
                ...
                if successor.hash() not in in_fringe and successor.hash() not in explored:
                    fringe[successor] = ignorePreconditions(
                        actions, successor, goal_state)
...
def get_successors(state, actions):
    ...
    for action in actions:
        if action.is_relevantForward(state):
            successor = State(
                state, action, state.positive_literals, state.negative_literals)
...
def ignorePreconditions(actions, state, goal):
    st = State(state.parent, state.action,
               state.positive_literals, state.negative_literals)
    cost = 0
    for act in actions:
